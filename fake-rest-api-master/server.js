...
server.post("/auth/register", (req, res) => {
	...
	server.get("/usuarios", (req, res) => {
		res.json({ usuarios: userdb.usuarios });
	});
	...
	fs.readFile("./usuarios.json", (err, data) => {
		...
		var last_item_id =
			data.usuarios.length > 0 ? data.usuarios[data.usuarios.length - 1].id : 0;
...
process.on("uncaughtException", function (err) {
	if (err.errno === "EADDRINUSE")
		console.log(
			"Atenção! Para rodar a Fake API, é esperado que a porta 8000 esteja livre para uso, mas ela já está sendo utilizada ."
		);
